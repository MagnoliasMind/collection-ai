...
class Harm(Effect):############################
    ...
    def apply(self,pokemon:"Pokemon")->None:
        pokemon.receive_damage(self.amount)
        print(f"{pokemon.name} receives {self.amount} harm ")
        
class Burning(Effect):
    name = "Burning"
    
    def __init__(self, amount: int,duration:int = 2)->None:
        super().__init__(duration)
        self.amount = self.amount
        
    def apply(self,pokemon:"Pokemon")->None:
        pokemon.receive_damage(self.amount)
        print(f"{pokemon.name} is burning , takes {self.amount} ")
...
class Defend(Effect):
    ...
    def apply(self,pokemon:"Pokemon",duration:int = 1,change:float = 1)->None :
        pokemon.defense += self.amount
        pokemon.defense *= change
